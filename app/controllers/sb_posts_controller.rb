class SbPostsController < BaseController
  ...
  uses_tiny_mce :only => [:edit, :update], :options => AppConfig.default_mce_options
  ...
  def search
    ...
    @posts = SbPost.with_query_options.paginate :all, :conditions => conditions, :page => params[:page]
  ...
  def create
    ...
    respond_to do |format|
      format.html do
        redirect_to forum_topic_path(:forum_id => params[:forum_id], :id => params[:topic_id], :anchor => dom_id(@post), :page => params[:page] || '1')
      end
